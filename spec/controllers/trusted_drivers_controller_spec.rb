...
RSpec.describe TrustedDriversController, type: :controller do
  describe 'routing' do
    ...
    it { expect(get: '/api/trusted_drivers').to route_to(controller: 'trusted_drivers', format: :json, action: 'trusted_drivers')}
    it { expect(get: '/api/trusted_for').to route_to(controller: 'trusted_drivers', format: :json, action: 'trusted_for')}
  end

  describe 'GET#trusted_drivers' do
    subject { create(:trusted_driver) }
    let(:send_request) { get :trusted_drivers, params: { format: JSON } }

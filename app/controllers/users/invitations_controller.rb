  ...
  class InvitationsController < ApplicationController
    ...
    def family_connection_params
      params.require(:user).permit(:member_type)
          .merge(requestor_user: current_user, receiver_user: receiver_user)
    end

    def receiver_user
      @receiver_user ||= User.where("phone = :phone or email = :email", {
          phone: params[:user][:phone], email: params[:user][:email]
      }).first
    end
  end
end
